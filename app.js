...
var getLists = (_, res) => __async(void 0, null, function* () {
  try {
...
var createList = (req, res) => __async(void 0, null, function* () {
  try {
    ...
    res.status(400).json({ message: "Unable to create this list.", error: error.message });
  }
});
var updateListName = (req, res) => __async(void 0, null, function* () {
  try {
    ...
    res.status(400).json({ message: "Unable to update list name.", error: error.message });
  }
});
var deleteListById = (req, res) => __async(void 0, null, function* () {
  try {
    const listId = req.params.listId;
    yield prisma.list.findUniqueOrThrow({
      where: {
        id: listId
      }
    });
    ...
    res.status(400).json({ message: "Unable to delete this list.", error: error.message });
  }
});
...
var toDoSchema = import_zod2.z.object({
  listName: import_zod2.z.string(),
  title: import_zod2.z.string()
});
var verifyListExistence = (listName) => {
  const list = prisma2.list.findUniqueOrThrow({
    where: {
      name: listName
    }
  });
...
var getToDos = (_, res) => __async(void 0, null, function* () {
  try {
    const toDos = yield prisma2.toDo.findMany();
    res.status(200).json(toDos);
  } catch (error) {
    res.status(500).json({ message: "Internal server error!" });
  }
});
var getToDosByListId = (req, res) => __async(void 0, null, function* () {
  try {
    const { listId } = req.body;
    const list = yield prisma2.list.findUniqueOrThrow({
      where: {
        id: listId
      }
    });
...
var createToDo = (req, res) => __async(void 0, null, function* () {
  try {
    const { listName, title } = toDoSchema.parse(req.body);
    const list = yield verifyListExistence(listName);
    const toDo = yield prisma2.toDo.create({
      data: {
        title,
        listId: list.id
      }
...
var updateToDo = (req, res) => __async(void 0, null, function* () {
  try {
    const updateToDoSchema = import_zod2.z.object({
      toDoId: import_zod2.z.string(),
      newTitle: import_zod2.z.string(),
      listName: import_zod2.z.string()
    });
    const { toDoId, newTitle, listName } = updateToDoSchema.parse(req.body);
    const list = yield verifyListExistence(listName);
    yield prisma2.list.update({
      where: {
        name: listName
      },
      data: {
...
var deleteToDo = (req, res) => __async(void 0, null, function* () {
  try {
    const deleteToDoSchema = import_zod2.z.object({
      toDoId: import_zod2.z.string()
    });
    const { toDoId } = deleteToDoSchema.parse(req.body);
    yield prisma2.toDo.findUniqueOrThrow({
      where: {
        id: toDoId
      }
    });
    yield prisma2.toDo.delete({
      where: {
        id: toDoId
      }
    });
...
var id = new import_bson.ObjectId();
...
app.get("/todosById", getToDosByListId);
